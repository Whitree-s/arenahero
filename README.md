...
本仓库附带两套可热切换的作战策略与一套实时地图监控，覆盖探索 / 巡逻 / 攻击 /
采集 / 信标 / 防御 / 迁移全流程，二者共享同一份地图记忆（`map.json`），切换不丢进度。

### 决策策略

| 策略 | 文件 | 风格 |
|---|---|---|
| 新决策 | `strategies/heuristic.py` | 集中式全局规划，按角色（先锋 / 游侠 / 工人）分发动作 |
| 旧决策 | `arena_agent.py` | 反应式流水线（`plan_turn_v2` 14 步） |

- **新决策**逻辑要点：采集（满载回仓 / 发现即采 / 集中派单 + 12 tick 粘性防抖 / 防死锁让位）；探索（工人环带扩张 / 先锋限距 / 覆盖潜力 rollout 选点）；巡逻（idle 三圈）；攻击（按敌方战斗单位聚类 → 派遣 2× 兵力，1 先锋 + 1 游侠配 1 敌、二打一不挤人 → 包围环布位 → **全员到位才正式开火**，未聚齐绝不提前少打多）；防御（固定守家预留 / 攻城屏障 / 威胁比例自毁腾人口）；信标就近抢占；资源枯竭饥饿迁移；记忆（资源视野反证清除、敌方单位 25 tick 遗忘）。
- **旧决策**逻辑要点：信标全图行军（人口达阈值即派最近工人、无论多远）；专职探索小队 + 80% 偏置；采集闭环；反应式战斗；按比例召回；V+R 叠组迷雾猎杀；工人回防封堵。

### 实时地图监控

`monitor.html` + `bridge_stream.py` 把 agent 每 tick 的状态流渲染为可缩放小地图：
探索迷雾（已探索常驻、未探索全黑）、单位实时朝向箭头、资源双层显示（当前视野
实心绿点 / 离开视野空心绿菱形，采空自动清除不留幽灵图标）、敌方核心红色五角星
标记、右侧事件流实时滚动（采集 / 交付 / 接敌 / 移动）。

### 参数调节

新决策各模式提供实时滑杆（搜索半径 / 探索半径），悬浮按钮面板位于地图左上角，
刷新页面不丢失。
